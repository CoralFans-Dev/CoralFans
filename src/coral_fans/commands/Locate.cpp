...
void registerLocateCommand(config::CommandConfigStruct& config) {
    if (config.enabled) {
        ...
        auto& locateCommand = ll::command::CommandRegistrar::getInstance(false).getOrCreateCommand(
            config.command,
            "command.locate.description"_tr(),
            config.permission
        );
    ...
    functions::locate::DuplicatableManager::hook(config.enabled);
}
} // namespace coral_fans::commands